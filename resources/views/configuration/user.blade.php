...
<form class="lw-ajax-form lw-form" method="post" action="<?= route('manage.configuration.write', ['pageType' => request()->pageType]) ?>">
    <fieldset class="mb-4" x-data="{isNewVendorRegistrationEnabled: {{ getAppSettings('enable_vendor_registration') ? 1 : 0 }} }">
        <legend>{{  __tr('New User Registration') }}</legend>
        <x-lw.checkbox :offValue="0" id="lwEnableVendorRegistration" name="enable_vendor_registration" data-lw-plugin="lwSwitchery" @click="isNewVendorRegistrationEnabled = !isNewVendorRegistrationEnabled" :checked="getAppSettings('enable_vendor_registration')" :label="__tr('Enable User Registration')" value="1" />
